    ...
    <div className="min-h-screen bg-black text-white">
      <div className="container mx-auto px-4 py-16 max-w-2xl">
        ...
        <div className="space-y-8">
          <RepositoryInput url={url} setUrl={setUrl} isPrivateRepo={isPrivateRepo} setIsPrivateRepo={setIsPrivateRepo} disabled={isProving || isVerifying || isCompressing} setError={setError} />

          {/* GitHub Token Input (visible only for private repos) */}
          <GitHubTokenInput visible={isPrivateRepo} githubToken={githubToken} setGithubToken={setGithubToken} disabled={isProving || isVerifying || isCompressing} />

          {/* Username Input */}
          <UsernameInput username={username} setUsername={setUsername} disabled={isProving || isVerifying || isCompressing} setError={setError} />

          {/* Action Buttons */}
          <ActionButtons onProve={handleProve} onVerify={handleVerify} onCompress={handleCompress} canAct={!error && !!username.trim()} hasPresentation={!!presentation} isProving={isProving} isVerifying={isVerifying} isCompressing={isCompressing} />

          {/* On-chain Verification Controls */}
          {zkProofResult && (
            <OnChainVerificationPanel zkProofResult={zkProofResult} username={username} inputUrl={url} setError={setError} />
          )}
