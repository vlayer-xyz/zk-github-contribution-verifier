...
ZK_PROVER_URL=http://localhost:3000
```

### Local Development with zk-prover-server

To use a locally running zk-prover-server instead of the remote API:

1. Start the zk-prover-server (runs on port 3000):

```bash
cd ../zk-prover-server
cargo run --bin zk-prover-server
```

2. The `ZK_PROVER_URL` environment variable defaults to `http://localhost:3000`, so the app will automatically use your local server for ZK proof compression.

3. The Next.js dev server runs on port 3001 to avoid conflicts with the zk-prover-server.
